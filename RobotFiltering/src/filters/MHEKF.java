...
import controllers.EKFControl;

public class MHEKF implements Filter {
	...
	public MHEKF(Control Controller, Data Dataset, Double initQ, Double initR) {
		defQ = initQ;
		defR = initR;
		FilterList.add(new EKF((EKFControl) Controller, Dataset, defQ, defR));
		NumFilters++;
		dDataset = Dataset;
	}
	...
	public void setControl(Control Controller) {
		...
		// them.
		if (!ProcessOn) {
			FilterList.add(new EKF(Controller, dDataset, defQ, defR));
			NumFilters++;
		} else {
			System.out.println("Can't add controllers to the MHEKF while its running!");
		}
	...
	public void setControl(Control Controller, Double initQ, Double initR) {
		...
		// them.
		if (!ProcessOn) {
			FilterList.add(new EKF(Controller, dDataset, initQ, initR));
			NumFilters++;
		} else {
			System.out.println("Can't add controllers to the MHEKF while its running!");
		}
	...
	public void startProcess() {
		...
		for (int i = 0; i<iLength; i++){
			...
			Matrix TotalResid = new Matrix (1, 3, 0.0);
...
//			System.out.println(TotalResid.get(0, 0) + "," + TotalResid.get(0, 1) + "," + TotalResid.get(0, 2));
			for (int j = 0; j<NumFilters; j++) {
				...
				tempWeight.set(0, 1, Resids.get(j).get(i).get(0, 1)/TotalResid.get(0, 1));
				tempWeight.set(0, 2, Resids.get(j).get(i).get(0, 2)/TotalResid.get(0, 2));
